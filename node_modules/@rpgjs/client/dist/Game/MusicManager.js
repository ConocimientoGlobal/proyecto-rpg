...
//#region src/Game/MusicManager.ts
var clampVolume = (value) => Math.max(0, Math.min(1, value));
var isSource = (value) => /^(https?:\/\/|\/|data:|blob:)/.test(value) || /\.(aac|flac|m4a|mp3|oga|ogg|opus|wav|webm)(\?.*)?$/i.test(value);
/**
...
*/
var RpgMusicManager = class {
	get outputGain() {
		return this.configuredOutputGain * Math.min(1, ...this.ducks.values());
	}
	constructor(host) {
		this.host = host;
		this.mapVolume = signal(1);
		this.sounds = /* @__PURE__ */ new Map();
		this.revision = 0;
		this.configuredOutputGain = 1;
		this.ducks = /* @__PURE__ */ new Map();
		this.suspensions = 0;
		this.pausedSounds = /* @__PURE__ */ new Set();
		this.currentBaseVolume = .8;
	...
	async enter(id, options = {}) {
		...
		if (fadePreviousFirst) this.fade(previous, this.readVolume(previous), 0, fadeOutMs);
		if ((fadeMapFirst || fadePreviousFirst) && fadeOutMs > 0) this.schedule(finishFadeOut, fadeOutMs, revision);
		else {
			this.setMapVolume(mapVolume);
			startNextTrack();
		}
	...
	*/
	pause() {
		if (this.suspensions++ === 0) {
			for (const sound of [this.mapSound, this.currentSound]) if (sound && sound.playing?.() !== false) {
				sound.pause?.();
		...
		return () => {
			...
			for (const sound of this.pausedSounds) if (sound === this.mapSound || sound === this.currentSound) sound.play?.();
	...
	async resolve(id) {
		...
		const sound = typeof src === "string" ? this.host.createSound(src, {
			loop: true,
			volume: 0
		}) : void 0;
	...
	fadeOutThenRestoreMap(duration, revision) {
	...
	fade(sound, from, to, duration) {
		if (duration > 0 && typeof sound.fade === "function") sound.fade(from, to, duration);
		else this.setVolume(sound, to);
	}
	readVolume(sound) {
	...
	fadeMapVolume(target, duration, revision) {
		...
		if (duration > 0) this.schedule(() => this.setVolume(this.mapSound, normalized * this.outputGain), duration, revision);
	}
	setMapVolume(value) {
	...
	schedule(callback, delay, revision) {
