//#region src/components/sprite-alpha-bounds.ts
var positiveDimension = (value) => typeof value === "number" && Number.isFinite(value) && value > 0 ? value : void 0;
/**
...
*/
var resolveTextureSourceDimensions = (value) => {
	const texture = value;
...
var alphaBoundsCache = /* @__PURE__ */ new Map();
var loadCachedSpriteSheetAlphaBounds = (key, loader) => {
	const cached = alphaBoundsCache.get(key);
	if (cached) return cached;
	const pending = loader().then((bounds) => {
...
var scanSpriteSheetAlphaBounds = (pixels, imageWidth, imageHeight, framesWidth, framesHeight) => {
	const frameWidth = Math.floor(imageWidth / framesWidth);
	const frameHeight = Math.floor(imageHeight / framesHeight);
	const bounds = Array.from({ length: framesWidth * framesHeight }, () => ({
		left: frameWidth,
		top: frameHeight,
		right: -1,
		bottom: -1
...
var mergeSpriteAlphaBounds = (values) => {
...
//#endregion
export { loadCachedSpriteSheetAlphaBounds, mergeSpriteAlphaBounds, resolveTextureSourceDimensions, scanSpriteSheetAlphaBounds };

//# sourceMappingURL=sprite-alpha-bounds.js.map