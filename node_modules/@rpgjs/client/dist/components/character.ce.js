...
import { applyCameraFollow, clearCameraFollowPlugins, ownsCameraFollow, ownsCameraFollowRevision, syncCameraFollowPosition } from "../services/cameraFollow.js";
import { RpgClientEngine } from "../RpgClientEngine.js";
import { observeCurrentPlayerControls } from "./current-player-controls.js";
import { resolveScaledGraphicBounds, resolveScaledHitboxAnchor, scaleHitboxForGraphicDisplay, toPositiveNumber } from "./character-hitbox.js";
import __ce_component$1 from "./player-components.ce.js";
import __ce_component$2 from "./interaction-components.ce.js";
import { loadCachedSpriteSheetAlphaBounds, mergeSpriteAlphaBounds, resolveTextureSourceDimensions, scanSpriteSheetAlphaBounds } from "./sprite-alpha-bounds.js";
import { Container, Sprite, animatedSignal, computed, cond, effect, h, loop, mount, on, signal, tick, untracked, useDefineEmits, useDefineProps, useProps } from "canvasengine";
...
function component($$props) {
	...
	const alphaFrameBounds = signal({});
	const loadingAlphaFrameBounds = /* @__PURE__ */ new Set();
	...
	const resolveAnimationFrames = (textureOptions) => {
		...
		try {
			...
			return (Array.isArray(firstGroup) ? firstGroup : frames).filter((frame) => frame && typeof frame === "object");
		} catch {
			return [];
		}
	};
	const resolveFirstAnimationFrame = (textureOptions) => resolveAnimationFrames(textureOptions)[0] ?? {};
	...
	const loadImageDimensions = (image) => {
		...
		Assets.load(source).then((texture) => {
			const dimensions = resolveTextureSourceDimensions(texture);
			if (!dimensions) return;
			imageDimensions.update((current) => ({
				...current,
				[source]: dimensions
			}));
		}).catch(() => {}).finally(() => {
	...
	const alphaBoundsKey = (textureOptions) => {
		const source = resolveImageSource(textureOptions?.image);
		const framesWidth = Math.floor(toPositiveNumber(textureOptions?.framesWidth) ?? 1);
		const framesHeight = Math.floor(toPositiveNumber(textureOptions?.framesHeight) ?? 1);
		return source ? `${source}|${framesWidth}x${framesHeight}` : void 0;
	};
	const readTexturePixels = (texture, width, height) => {
		const resource = texture?.source?.resource ?? texture?.baseTexture?.resource?.source ?? texture?.source?.resource?.source;
		if (!resource) return void 0;
		const context = (typeof OffscreenCanvas !== "undefined" ? new OffscreenCanvas(width, height) : typeof document !== "undefined" ? Object.assign(document.createElement("canvas"), {
			width,
			height
		}) : void 0)?.getContext?.("2d", { willReadFrequently: true });
		if (!context) return void 0;
		context.clearRect(0, 0, width, height);
		context.drawImage(resource, 0, 0, width, height);
		return context.getImageData(0, 0, width, height).data;
	};
	const loadAlphaBounds = (textureOptions) => {
		if (textureOptions?.trimTransparentBounds !== true) return;
		const source = resolveImageSource(textureOptions?.image);
		const key = alphaBoundsKey(textureOptions);
		if (!source || !key || Object.prototype.hasOwnProperty.call(alphaFrameBounds(), key) || loadingAlphaFrameBounds.has(key)) return;
		loadingAlphaFrameBounds.add(key);
		loadCachedSpriteSheetAlphaBounds(key, async () => {
			...
			const pixels = readTexturePixels(texture, width, height);
			if (!pixels) return null;
			return scanSpriteSheetAlphaBounds(pixels, width, height, framesWidth, framesHeight);
		}).then((bounds) => {
			alphaFrameBounds.update((current) => ({
				...current,
				[key]: bounds
			}));
		}).catch(() => {
			...
			loadingAlphaFrameBounds.delete(key);
		});
	};
	const resolveVisibleFrameBounds = (textureOptions, boundsByKey) => {
		if (textureOptions?.trimTransparentBounds !== true) return void 0;
		const key = alphaBoundsKey(textureOptions);
		const allBounds = key ? boundsByKey[key] : void 0;
		if (!Array.isArray(allBounds)) return void 0;
		const framesWidth = Math.floor(toPositiveNumber(textureOptions?.framesWidth) ?? 1);
		const framesHeight = Math.floor(toPositiveNumber(textureOptions?.framesHeight) ?? 1);
		const frames = resolveAnimationFrames(textureOptions);
		return mergeSpriteAlphaBounds((frames.length > 0 ? frames : [{
			frameX: 0,
			frameY: 0
		}]).map((frame) => {
			const frameX = Math.min(framesWidth - 1, Math.max(0, Math.floor(toFiniteNumber(frame.frameX, 0))));
			const frameY = Math.min(framesHeight - 1, Math.max(0, Math.floor(toFiniteNumber(frame.frameY, 0))));
			return allBounds[frameY * framesWidth + frameX];
		}));
	};
	effect(() => {
		const sources = /* @__PURE__ */ new Set();
		graphicsSignals().forEach((graphicObject) => {
			loadAlphaBounds(resolveTextureOptions(graphicObject));
	...
	const graphicBounds = computed(() => {
		...
		const loadedAlphaBounds = alphaFrameBounds();
		...
		graphics.forEach((graphicObject) => {
			...
			const graphic = resolveScaledGraphicBounds({
				frameWidth: spriteWidth,
				frameHeight: spriteHeight,
				visibleBounds: resolveVisibleFrameBounds(textureOptions, loadedAlphaBounds),
				anchor,
				scale,
				x,
				y
	...
	sprite.__rpgjsGraphicBounds = graphicBounds;
	...
	mount((element) => {
		...
		let appliedCameraFollowOwnership = null;
		const cameraFollowTarget = {
			...
			get destroyed() {
				return element.componentInstance?.destroyed ?? true;
			}
		};
		if (typeof document !== "undefined") {
		...
		controlsSubscription = observeCurrentPlayerControls(isCurrentPlayer, element.directives.controls, (controls) => client.setKeyboardControls(controls));
		effect(() => {
			const followRevision = client.cameraFollowRevision();
			if (!shouldFollowCamera()) {
				appliedCameraFollowRevision = null;
				appliedCameraFollowOwnership = null;
				return;
			}
			const smoothMove = client.cameraFollowSmoothMove;
			const viewport = element.props.context?.viewport;
			if (!viewport || !element.componentInstance) {
				appliedCameraFollowRevision = null;
				appliedCameraFollowOwnership = null;
				return;
			}
			appliedCameraFollowOwnership = untracked(() => applyCameraFollow({
				viewport,
			...
			appliedCameraFollowRevision = appliedCameraFollowOwnership ? followRevision : null;
		});
		effect(() => {
			if (!shouldFollowCamera() || !ownsCameraFollow(appliedCameraFollowOwnership)) return;
			const viewport = element.props.context?.viewport;
			if (!viewport || !element.componentInstance) return;
			syncCameraFollowPosition(viewport, cameraFollowTarget, client.cameraFollowSmoothMove);
		});
		return () => {
			controlsSubscription?.unsubscribe();
			if (ownsCameraFollowRevision(appliedCameraFollowRevision, client.cameraFollowRevision())) clearCameraFollowPlugins(element.props.context?.viewport, appliedCameraFollowOwnership);
		};
	});
